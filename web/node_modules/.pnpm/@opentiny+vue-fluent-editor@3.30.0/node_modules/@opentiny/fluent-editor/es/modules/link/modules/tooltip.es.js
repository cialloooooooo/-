...
const _Tooltip = class _Tooltip extends BaseTooltip {
  constructor(quill, bounds) {
    ...
    this.isHover = false;
    this.resolveOptions();
    Link$1.autoProtocol = this.options.autoProtocol;
  ...
  handleMouseEnter(event) {
    ...
    const preview = Link$1.formats(linkNode);
  ...
  listen() {
    ...
    this.quill.root.addEventListener(
      "mouseover",
      (event) => {
        if ((event.target.tagName.toUpperCase() !== "A" || !event.target.classList.contains(Link$1.className)) && !event.target.closest(`a.${Link$1.className}`)) {
          return;
        }
    ...
    this.quill.root.addEventListener(
      "mouseout",
      (event) => {
        if (event.target.tagName.toUpperCase() !== "A" && !event.target.closest(`a.${Link$1.className}`)) {
          return;
        }
    ...
    this.root.querySelector("a.ql-preview").addEventListener("click", (event) => {
      const link = Link$1.sanitize(this.textbox.value);
