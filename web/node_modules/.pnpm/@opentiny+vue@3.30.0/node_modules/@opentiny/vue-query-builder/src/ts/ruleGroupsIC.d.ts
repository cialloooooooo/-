import { DefaultCombinatorName, RuleType, DefaultRuleGroupArray, DefaultRuleType, RuleGroupArray, DefaultRuleGroupType, RuleGroupType } from './ruleGroups';
import { MappedTuple } from './ruleGroupsIC.utils';
export type RuleGroupTypeIC<R extends RuleType = RuleType, C extends string = string> = Omit<RuleGroupType<R, C>, 'combinator' | 'rules'> & {
...
export type RuleGroupICArray<RG extends RuleGroupTypeIC = RuleGroupTypeIC, C extends string = string, R extends RuleType = RuleType> = [R | RG] | [R | RG, ...MappedTuple<[C, R | RG]>] | ((R | RG)[] & {
    length: 0;
});
